...
//         logger (LOG_ERROR, __func__, "libpq error: PQstatus(psql) !=
//         CONNECTION_OK", NULL); logger (LOG_ERROR, __func__, "PSQL Error
//         Message: '%s'", PQerrorMessage(psql)); return -1;
//     }
//     else
//     {
//         logger (LOG_DBASE, __func__, "Connection made to the database",
//         NULL); return 0;
//     }
// }

// static int database_insert_postsql_weather_data (char *table, weather_t
// *data)
// {
//     PGresult *dbResult;
//     int ret = 0;
...
//               in_temperature," "out_temperature, out_dew_temperature,
//               windchill_temperature, wind_speed"
//               ", wind_gust, wind_direction, pressure, rel_pressure,
//               rain_last_hour"
//               ", rain_last_24h, rain_total, bytes) values (now(), %d, %d,
//               %0.1f, %0.1f" \
//               ", '%0.1f', '%0.1f', '%0.1f', '%0.1f', '%s', '%0.1f', '%0.1f',
//               '%0.1f','%0.1f', '%0.1f', '%s');", table, data->in_humidity,
//               data->out_humidity,
//               get_temperature(data->in_temp, UNIT_TYPE_IS_METRIC),
//               get_temperature(data->out_temp, UNIT_TYPE_IS_METRIC),
//               get_dew_point (data->out_temp, data->out_humidity,
//               UNIT_TYPE_IS_METRIC), get_wind_chill (data,
//               UNIT_TYPE_IS_METRIC), get_wind_speed(data->wind_speed,
//               UNIT_TYPE_IS_METRIC), get_wind_speed(data->wind_gust,
//               UNIT_TYPE_IS_METRIC), get_wind_direction (data->wind_dir,
//               WIND_AS_TEXT), get_pressure (data->abs_pressure), get_pressure
//               (data->rel_pressure), get_rainfall (data->last_hour_rain_fall,
//               UNIT_TYPE_IS_METRIC), get_rainfall (data->last_24_hr_rain_fall,
//               UNIT_TYPE_IS_METRIC), get_rainfall (data->total_rain_fall,
//               UNIT_TYPE_IS_METRIC), data->readBytes);
//
...
//    else
//    {
//        logger (LOG_ERROR, __func__, "Command Failed:: %s",
//        PQerrorMessage(psql)); ret = -1;
//    }
//    free (queryString);
//
//    return ret;
...
static int connect_to_mysql (dbase_config_t * dbconfig)
{
   ...
   asprintf (
      &debug_con_string,
      "host=%s dbname=%s port=%d user=%s password=%s",
      dbconfig->host,
      dbconfig->dbname,
      dbconfig->port,
      dbconfig->user,
...
{
   /* TODO: If enough interest, could do a switch here to connect to postgresql
...
{
   /* TODO: If enough interest, could do a switch here to insert to postgresql
    * database server instead */
...
int validate_db_config (dbase_config_t * dbase_config)
{
   ...
   while (field < DB_FIELD_MAX && ret >= 0)
   {
      ...
      case DB_PORT:
         if (dbase_config->port == 0)
         {
            logger (
               LOG_ERROR,
               __func__,
               "Config file is missing option %s",
               "port");
            ret = -1;
         }
         break;
      ...
      case DB_PASSWORD:
         RETURN_IF_ERROR (
            check_config_value ("password", dbase_config->password));
         break;
      default:
         ret = -1;
      }
...
static void database_copy_config_value (char * src, char ** dest, char * name)
{
   ...
   if (asprintf (dest, "%s", src))
   {
      logger (
         ...
         "Found %s=`%s`",
         name,
         strcmp ("password", name) == 0 ? secret : src);
   }
}
...
int database_init (FILE * config_file, dbase_config_t * dbase_config)
{
   ...
   while (fgets (line, sizeof (line), config_file) != NULL)
   {
      ...
      if (line[0] != '#')
      {
         ...
         if (strstr (line, "host"))
         {
            i > 0 ? database_copy_config_value (
                       _buffer,
                       &dbase_config->host,
                       "host")
                  : NULL;
